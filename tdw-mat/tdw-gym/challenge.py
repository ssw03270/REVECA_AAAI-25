...
class Challenge:
    ...
    def submit(self, agents, logger, eval_episodes):
        ...
        for agent_id, agent in enumerate(agents):
            cost[agent_id] = agent.LLM.total_cost
        ...
        for i, episode in enumerate(eval_episodes):
            ...
            for agent_id, agent in enumerate(agents):
                cost += agent.LLM.total_cost
                message_cost = len(agent.dialogue_history)
            print(agents[0].dialogue_history)
            ...
            result = {
                ...
                'dialogue':agents[0].dialogue_history
            }
