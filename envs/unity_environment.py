...
class UnityEnvironment(BaseUnityEnvironment):
    ...
    def __init__(self,
        ...
        self.container_classes = [
            ...
            'dishwasher',
            'microwave']
        self.detection_all_object = ['plate', 'coffeetable', 'poundcake', 'wine', 'juice', 'pudding', 'apple',
                                     'pancake', 'cutleryfork', 'kitchentable', 'cupcake',
                                     'coffeepot'] + self.container_classes + ['character']
        ...
        for i, action in enumerate(actions):
            ...
            if action == 'walktowards' and self.keep_move_steps[i] > 4:
                if self.range_distance(self.location[i][max(0, self.steps - 4): self.steps + 1]) < 0.5:
                    ...
                    random_flag = True
            self.keep_move_steps[i] = self.keep_move_steps[
                                          i] + 1 if action == 'walktowards' and random_flag == False else 0
    ...
    def get_observations(self):
        ...
        for agent_id in range(self.num_agents):
            ...
            if ('image' in obs_type or self.data_collection):
                        ...
                        for j in range(image_width):
                            ...
                            if (ids[t, i, j] != -1):
                                if (ids[t, i, j] not in inside_info.keys()):
                                    inside_info[ids[t, i, j]] = [edge['to_id'] for edge in self.full_graph['edges'] if
                                                                 edge['from_id'] == ids[t, i, j] and edge[
                                                                     'relation_type'] == 'INSIDE'][0]
                                if (self.dict_graph[inside_info[ids[t, i, j]]]['category'] not in [
                                    'Rooms'] and 'CLOSED' in self.dict_graph[inside_info[ids[t, i, j]]]['states']):
                                    ids[t, i, j] = -1
                ...
                if self.save_image:
                    ...
                    for i in range(len(camera_ids)):
                        ...
                        cv2.imwrite(os.path.join('saved_images', f"{agent_id}_{self.steps:03}_seg_reference.png"),
                                    rot_colorids)
            ...
            if self.data_collection:
                        ...
                        for j in range(image_width):
                            ...
                            if ids_class[t, i, j] != -1:
                                if ids_class[t, i, j] not in inside_info.keys():
                                    inside_info[ids_class[t, i, j]] = \
                                    [edge['to_id'] for edge in self.full_graph['edges'] if
                                     edge['from_id'] == ids_class[t, i, j] and edge['relation_type'] == 'INSIDE'][0]
                                if self.dict_graph[inside_info[ids_class[t, i, j]]]['category'] not in [
                                    'Rooms'] and 'CLOSED' in self.dict_graph[inside_info[ids_class[t, i, j]]]['states']:
                                    ids[t, i, j] = -1
                            if ids_class[t, i, j] != -1:
                ...
                color_id_instance = np.stack(
                    ((ids_instance % 10) * 10, ((ids_instance // 10) % 10) * 10, ((ids_instance // 100) % 10) * 10),
                    axis=3)
                if 'bgr' not in dict_observations[agent_id].keys():
                    s, dict_observations[agent_id]['bgr'] = self.comm.camera_image(camera_ids, mode='normal',
                                                                                   image_width=image_width,
                                                                                   image_height=image_height)
                if os.path.exists(
                        self.data_collection_dir + str(self.global_episode_id) + '_' + str(self.env_id) + '/') == False:
                    os.mkdir(self.data_collection_dir + str(self.global_episode_id) + '_' + str(self.env_id) + '/')
                import cv2
                for t in range(len(camera_ids)):
                    cv2.imwrite(
                        self.data_collection_dir + str(self.global_episode_id) + '_' + str(self.env_id) + '/' + str(
                            self.steps) + '_' + str(agent_id) + '_ids_instance.png', ids_instance[t])
                    cv2.imwrite(
                        self.data_collection_dir + str(self.global_episode_id) + '_' + str(self.env_id) + '/' + str(
                            self.steps) + '_' + str(agent_id) + '_color_ids_instance.png', color_id_instance[t])
                    cv2.imwrite(
                        self.data_collection_dir + str(self.global_episode_id) + '_' + str(self.env_id) + '/' + str(
                            self.steps) + '_' + str(agent_id) + '_ids_class.png', ids_class[t])
                    cv2.imwrite(
                        self.data_collection_dir + str(self.global_episode_id) + '_' + str(self.env_id) + '/' + str(
                            self.steps) + '_' + str(agent_id) + '_rgb.png', dict_observations[agent_id]['bgr'][t])
    ...
    def bbox_2d_to_id(self, bbox, agent_id, frame_id=0, mask=None, threshold=0.3):
        r'''
			...
			return: id and name of the object, if the main object is not relative object, return (-1, None).
		'''
        id_array = copy.deepcopy(
            self.id_map[agent_id][frame_id, int(bbox[1]): int(bbox[3]), int(bbox[0]): int(bbox[2])])
        if (mask is not None):
    ...
    def id_to_inside_room(self, id):
        ...
        while (id not in self.all_room_id):
            id = [edge['to_id'] for edge in self.full_graph['edges'] if
                  edge['from_id'] == id and edge['relation_type'] == 'INSIDE'][0]
        return id
    ...
    def nodes_in_same_room(self, nodes, agent_id=0):
        same_room_node = [node for node in nodes if
                          self.id_to_inside_room(node['id']) == self.id_to_inside_room(agent_id)]
        not_inside_node = []
        for node in same_room_node:
            inside_info = [edge['to_id'] for edge in self.full_graph['edges'] if
                           edge['from_id'] == node['id'] and edge['relation_type'] == 'INSIDE']
            assert (len(inside_info) <= 1)
            if len(inside_info) == 0:
                not_inside_node.append(node)
            elif self.dict_graph[inside_info[0]]['category'] in ['Rooms'] or 'CLOSED' not in \
                    self.dict_graph[inside_info[0]]['states']:
                not_inside_node.append(node)
    ...
    def remove_duplicate_graph(self, graph):
        ...
        for edge in graph['edges']:
            if edge['from_id'] in allnode and (
                    edge['to_id'] in allnode or self.id2node[edge['to_id']]['category'] == 'Rooms'):
    ...
    def clean_object_relationship(self, graph):
        return {
            ...
            'edges': [edge for edge in graph['edges'] if (
                        self.id2node[edge['from_id']]['class_name'] == 'character' or self.id2node[edge['to_id']][
                    'class_name'] == 'character') and edge['relation_type'] not in ['ON', 'BETWEEN', 'INSIDE']],
        }
    ...
    def get_observation(self, agent_id, obs_type):
        ...
        if obs_type == 'partial':
            ...
            if self.save_image:
                ...
                s, bgr_images = self.comm.camera_image(camera_ids, mode='normal', image_width=image_width,
                                                       image_height=image_height)
                s, depth_images = self.comm.camera_image(camera_ids, mode='depth', image_width=image_width,
                                                         image_height=image_height)
                ...
                if self.save_image:
                    ...
                    for i in range(len(camera_ids)):
                        ...
                        cv2.imwrite(os.path.join('saved_images', f"{agent_id}_{self.steps:03}_depth.png"),
                                    50 / rot_depth)  # for visualization
