...
if __name__ == '__main__':
    ...
    for iter_id in range(num_tries):
        ...
        for episode_id in test_episodes:
            ...
            success, steps, saved_info, move_distance = arena.run()
            steps -= 1
        ...
        print('average each distance (finishing the tasks):', np.mean(distance_list, axis=0) if len(distance_list) > 0 else None)
        print('average distance (finishing the tasks):', np.mean(distance_list) if len(distance_list) > 0 else None)
