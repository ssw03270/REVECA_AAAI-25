...
def clean_graph(state, goal_spec, last_opened, additional_objects):
    new_graph = {}
    # get all ids
    ids_interaction = []
    nodes_missing = []
    ...
    ids_interaction += additional_objects
    new_graph = {
...
def get_plan(additional_objects, sample_id, root_action, root_node, env, mcts, nb_steps, goal_spec, res, last_subgoal, last_action,
             opponent_subgoal=None, verbose=True):
    init_state = env.state
    if True:  # clean graph
        init_state = clean_graph(init_state, goal_spec, mcts.last_opened, additional_objects)
...
class MCTS_agent:
    ...
    def filtering_graph(self, graph):
        ...
        roomname = [edge['to_id'] for edge in graph['edges'] if
                    edge['from_id'] == self.agent_id and edge['relation_type'] == 'INSIDE'][0]
        roomname = id2node[roomname]['class_name']
        for edge in graph['edges']:
            if len(self.additional_objects[roomname]) < num_additional_objects:
                if set(id2node[edge['to_id']]['properties']) == {'GRABBABLE', 'MOVABLE'} and edge['to_id'] not in self.additional_objects[roomname]:
                    if id2node[edge['to_id']]['class_name'] == "pillow":
                        continue

                    self.additional_objects[roomname].append(edge['to_id'])

            if edge['to_id'] in self.additional_objects[roomname] or edge['from_id'] in self.additional_objects[roomname]:
                new_edges.append(edge)
    ...
    def filter_msg_with_target_class(self, goal_classes, id_to_class):
        try:
            self.belief.message_to_send = [msg for msg in self.belief.message_to_send if
                                           id_to_class[msg[1]] in goal_classes]
        except KeyError:
            print('----------------------------------------------')
    ...
    def record_action(self):
        # 기록
        dir_path = f"../test_results_mcts/action_list/{self.task_id}"
        if not os.path.exists(dir_path):
            os.makedirs(dir_path)
        file_path = os.path.join(dir_path, f'{self.agent_id}_{self.step}.txt')
        with open(file_path, 'w') as file:
            file.write(self.last_action)

    def record_message(self, message, oppo_message):
        # 기록
        dir_path = f"../test_results_mcts/message_list/{self.task_id}"
        if not os.path.exists(dir_path):
            os.makedirs(dir_path)
        file_path = os.path.join(dir_path, f'{self.LLM.agent_name}_{self.steps}.txt')
        with open(file_path, 'w') as file:
            file.write(message + '\n')
            file.write(oppo_message)
    ...
    def get_action(self, obs, goal_spec, opponent_subgoal=None):
        ...
        if obs['messages'] != None and len(obs['messages']) > 1 and obs['messages'][1 - self.char_index]:
            if self.char_index == 0 and obs['messages'][0] != None:
                msg = {}  # do not recurrent
            else:
            ...
            if self.satisfied_comm:
                if 'E' in msg.keys():
                    ...
                    if msg['E']['relation_type'] == 'INSIDE':
                        add_relation['edges'] = [
                            {'from_id': msg['E']['from_id'], 'to_id': msg['E']['to_id'], 'relation_type': 'INSIDE'},
                            {'from_id': msg['E']['to_id'], 'to_id': msg['E']['room_id'], 'relation_type': 'INSIDE'}]
                    elif msg['E']['relation_type'] == 'ON':
                        add_relation['edges'] = [
                            {'from_id': msg['E']['from_id'], 'to_id': msg['E']['to_id'], 'relation_type': 'ON'},
                            {'from_id': msg['E']['to_id'], 'to_id': msg['E']['room_id'], 'relation_type': 'INSIDE'},
                            {'from_id': msg['E']['from_id'], 'to_id': msg['E']['room_id'], 'relation_type': 'INSIDE'}]
                    relative_id = list(set([x['from_id'] for x in add_relation['edges']] + [x['to_id'] for x in
                                                                                            add_relation['edges']]))
                    add_relation['nodes'] = [
        ...
        id2node = {x['id']: x for x in obs['nodes']}
        roomname = [edge['to_id'] for edge in obs['edges'] if
                    edge['from_id'] == self.agent_id and edge['relation_type'] == 'INSIDE'][0]
        roomname = id2node[roomname]['class_name']
        additional_objects = self.additional_objects[roomname]
        plan, root_node, subgoals = get_plan(additional_objects, None, root_action, root_node, self.sim_env, self.mcts, nb_steps, goal_spec,
                                             None, last_subgoal, last_action, opponent_subgoal, verbose=verbose)
        if self.opponent_subgoal == 'comm' and self.received_opponent_subgoal is not None and \
                self.received_opponent_subgoal[-1] == '0': self.received_opponent_subgoal = None
        ...
        roomname = [edge['to_id'] for edge in self.sim_env.state['edges'] if
                    edge['from_id'] == self.agent_id and edge['relation_type'] == 'INSIDE'][0]
        ...
        id_to_class = {self.sim_env.state['nodes'][i]['id']: self.sim_env.state['nodes'][i]['class_name'] for i in
                       range(len(self.sim_env.state['nodes']))}
        ...
        if self.last_position != None:
            distance = math.sqrt((obs['location'][0] - self.last_position[0]) ** 2 +
                                 (obs['location'][2] - self.last_position[2]) ** 2)
            self.move_distance += distance
        self.move_history.append(obs['location'])
